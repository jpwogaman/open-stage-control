...
module.exports = {
    s: {
        alias: 'send',
        type: 'array',
        describe: 'default targets for all widgets (ip: port / domain: port / midi: port_name pairs)',
        check: (s) => {
    ...
    p: {
        ...
        check: (p) => {
            return !isNaN(p) && p > 1023 && parseInt(p) === p ? true : 'Port must be an integer >= 1024'
        }
    },
    o: {
        alias: 'osc-port',
        ...
        check: (o) => {
            return !isNaN(o) && o > 1023 && parseInt(o) === o ? true : 'Port must be an integer >= 1024'
        }
    },
    'tcp-port': {
        type: 'number',
        ...
        check: (t, argv) => {
            ...
            return !isNaN(t) && t > 1023 && parseInt(t) === t ? true : 'Port must be an integer >= 1024'
        }
    },
    'tcp-targets': {
        type: 'array',
        describe: 'tcp servers to connect to (ip: port pairs), does not substitute for --send',
        check: (s, argv) => {
            if (!argv['tcp-port']) return '--tcp-port must be set'
            return s.some((item) => !item.match(/^[^:]*:[0-9]{4,5}$/)) ? 'Targets must be ip:port or hostname:port pairs (port must be >= 1024)' : true
        }
    },
    m: {
        alias: 'midi',
    ...
    'client-options': {
        ...
        check: (o) => {
            return o.some((item) => !item.match(/^[^=]*=[^=]*$/)) ? 'Options must be key=value pairs' : true
        }
    },
    ...
    'remote-saving': {
        type: 'string',
        describe: "disable remote session saving for hosts that don't match the regular expression",
        check: (r, argv) => {
            var msg = true
            try {
                RegExp(r)
    ...
    authentication: {
        type: 'string',
        describe: 'restrict access to "user:password" (remote clients will be prompted for these credentials)',
        check: (a, argv) => {
    ...
    cubaseKeyCommands: {
        alias: 'cubase-keys',
        type: 'string',
        file: { name: 'Key Commands (.json)', extensions: ['json'] },
        describe: 'Cubase Key Commands File (.json)',
        check: (arg) => {
            return fs.existsSync(arg) ? true : 'Key commands file not found: ' + arg
        }
    },
    ...
    'client-position': {
        ...
        launcher: false,
        check: (s) => {
            return !s.match(/^[0-9]+,[0-9]+$/) ? 'Position must a x,y pair of integers' : true
        }
    },
    'client-size': {
        type: 'string',
        ...
        launcher: false,
        check: (s) => {
            return !s.match(/^[0-9]+,[0-9]+$/) ? 'Size must a x,y pair of integers' : true
        }
    },
